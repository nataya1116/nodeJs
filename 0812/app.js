...
app.post("/refresh", (req, res) => {
  ...
  if (req.cookies?.refresh) {
    ...
    jwt.verify(refreshtoken, process.env.REFRESH_TOKEN_KEY, (err, decode) => {
      // err가 나올 경우 유효하지 않은 토큰이므로 다시 로그인하게 한다.
      if(err){
        console.error(err);
        ...
        const accessToken = jwt.sign(
          {
            id : user.id  // id 외에도 필요한 데이터가 있다면 저장
          },
          process.env.ACCESS_TOKEN_KEY,
          {
            expiresIn: "10m"
...
// 액세스 토큰을 왜 짧게 하고 새로 로그인 정보를 갱신해줄까 해커가 악의적으로 액세스 토큰을 탈취하였을 때 로그인이 이미 되어 있는 상태라 막기가 어려워서 액세스 토큰의 유효기간을 짧게하고 리프레시 토큰의 유효기간을 길게 해서 사용자가 로그인을 자주 하는 불편함을 보완해준다. 리프레쉬 토큰을 다시 확인 시켜서 악의적으로 탈취된 액세스 토큰을 갱신해주는 역할도 해준다. 좀 더 해킹이 어려워지게 한다.